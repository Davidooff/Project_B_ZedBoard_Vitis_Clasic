...
void xSendUart(void *pvParameters) {
	...
	const int complit = UartClass.send(temperatureData->data, temperatureData->count);
	xil_printf("Uart task is compliet with code: %d\r\n", complit);
	while(complit){
	...
	while(1){
		if (*transmitData->temperatureCount == QUEUE_SIZE){
			...
			if(xSandUartTask){
				...
				xil_printf("The uartTask state is: %d\n", xTaskDetails.eCurrentState);
				if (xTaskDetails.eCurrentState == eRunning || xTaskDetails.eCurrentState == eReady){
					xil_printf("Uart task is incompliet. Killing task \r\n");
			...
			if(xSaveSdTask){
				...
				if (xTaskDetails.eCurrentState == eRunning || xTaskDetails.eCurrentState == eReady){
					xil_printf("SD task is incompliet. Killing task \r\n");
					vTaskDelete( xSandUartTask );
				}
			} else {
				xil_printf("Uart task wasn't created. Moving on \n\r");
